...
class Settings(BaseSettings):
    ...
    allowed_origins: str = Field(default_factory=lambda: os.getenv("ALLOWED_ORIGINS", "https://app.Shasit.com,https://shasit.com,http://localhost:3000"))
    allowed_hosts: str = Field(default_factory=lambda: os.getenv("ALLOWED_HOSTS", "api.aicomplyhub.com,localhost,127.0.0.1,testserver"))
    ...
    max_request_size_bytes: int = Field(default_factory=lambda: int(os.getenv("MAX_REQUEST_SIZE_BYTES", "26214400")))
    security_headers_enabled: bool = Field(default_factory=lambda: os.getenv("SECURITY_HEADERS_ENABLED", "true").lower() == "true")
    hsts_enabled: bool = Field(default_factory=lambda: os.getenv("HSTS_ENABLED", "true").lower() == "true")
    hsts_max_age_seconds: int = Field(default_factory=lambda: int(os.getenv("HSTS_MAX_AGE_SECONDS", "63072000")))
    trust_proxy_headers: bool = Field(default_factory=lambda: os.getenv("TRUST_PROXY_HEADERS", "true").lower() == "true")
    client_ip_header: str = Field(default_factory=lambda: os.getenv("CLIENT_IP_HEADER", "CF-Connecting-IP"))
    waf_provider: str = Field(default_factory=lambda: os.getenv("WAF_PROVIDER", "cloudflare"))
